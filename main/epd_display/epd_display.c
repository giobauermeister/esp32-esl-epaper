...
void epd_test(void) {
    ...
    epd_write_reg(0x10);  // Old frame buffer
    for (int i = 0; i < 12480; i++) {
        epd_write_data8(0xAA);  // Alternating pixels (test pattern)
    }

    epd_write_reg(0x13);  // New frame buffer
    for (int i = 0; i < 12480; i++) {
        epd_write_data8(0x55);  // Alternating pixels
    }
